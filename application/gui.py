...
class Gui(Frame):
    ...
    def write_config_file(self, event):
        self.write_gcode(config_file=True)
        config_file = "config.ngc"
    ...
    def write_gcode(self,config_file=False):
        '''
        ...
        if self.cut_type.get() == "engrave" or self.bit_shape.get() == "FLAT":
            ...
            order_out = geometry.sort_paths(ecoords)  # TODO
    ...
    def WRITE_CLEAN_UP(self, bit_type="straight"):
        ...
        while (rough_again == True or roughing == True):
            ...
            order_out = geometry.sort_paths(coords_out, 3)  # TODO
            new_coords = []
            for line in order_out:
                temp = line
                if (temp[0] < temp[1]):
    ...
    def menu_File_Save_G_Code_File(self):
        ...
        #TODO how to pass coords? separate parameter?
    ...
    def V_Carve_It(self, clean_flag=0, DXF_FLAG=False):
        ...
        if self.cut_type.get() == "v-carve" and self.fontdex.get() == False:
            ...
            self.model.set_accuracy(self.accuracy.get())
            self.model.setBatch(self.batch.get())
            self.model.set_plotscale(self.pscale)
